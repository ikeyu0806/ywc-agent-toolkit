...
Canonical directory layout, routing rules, naming, anti-patterns, and per-folder conventions for `docs/`. Used by `ywc-project-docs` when generating Korean or Japanese project documentation.
...
- Lowercase ASCII + hyphen only. Do not use Korean, Japanese, or Chinese filenames.
- Minimize suffixes such as `-architecture`, `-system`, and `-design` because the directory already conveys role.
  - Use `architecture/authentication.md`, not `authentication-architecture.md`.
  - Use `manuals/deploy-checklist.md`, not `setup-deploy-checklist.md`.
- Use noun-form subjects, not verbs.
...
- Include a table of contents when there are 5 or more sections.
- Use section numbering: `## N. Title`, `### N.M Title`.
...
3. Folder meaning should precede document title: LLMs interpret path together with title.
4. Do not duplicate the same content across folders: keep one source and link from elsewhere.
5. Implementation-baseline documents are rule-centric, not narrative: especially `specification/` documents prioritize expected behavior, constraints, edge cases, data contracts, and acceptance criteria.
...
7. Future plans: unimplemented features, linked to `docs/plans/`.
...
`product/` documents are the starting point for understanding why a feature exists before implementation. Include:
...
1. Existing document check: if a document on the same topic already exists, update it instead of creating a new one.
2. Directory selection: confirm the appropriate directory using the routing rules above.
3. Cross-references: when related documents exist, add bidirectional links where appropriate.
4. Official vs draft: pre-confirmation content goes in `todo/`; confirmed content moves to the official folder.
5. Explicit request: generate documents only when the user explicitly requests documentation work.
