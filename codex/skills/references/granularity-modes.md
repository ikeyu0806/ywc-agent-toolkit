...
This document specifies the two Granularity Modes supported by `ywc-task-generator` and the rules that differ between them. The executor **must** ask the user which mode to use before task decomposition — there is no silent default.
...
Use `llm` mode when:
- Tasks will be executed by an LLM agent (Codex, Claude Code, or similar)
