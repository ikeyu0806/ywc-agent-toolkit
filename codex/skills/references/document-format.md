...
| Principle | Rationale |
|-----------|-----------|
| Single file | Agents can inject the full vocabulary from one stable context file |
| Table-first | Tables are token-efficient for LLM context and easy for humans to scan |
| Bounded Context sections | Prevent homonym collisions across domain boundaries |
| Synonyms to Avoid column | Tells the LLM what not to generate |
...
Write this from the domain's perspective, not the technical implementation's.]
...
| Service | Stateless operation that does not fit Entity or Value Object |
...
ShopBot is a B2C e-commerce platform where Customers browse a Catalog, add Products to a Cart, and place Orders for fulfillment by the Warehouse team.

## Bounded Contexts

| Context | Responsibility |
|---------|---------------|
| Core Domain | All domain concepts for a single-context project |
...
| Date | Mode | Summary |
|------|------|---------|
| 2026-05-03 | new | Initial creation - 6 terms, Core Domain |
```
...
For Codex-oriented repositories, reference the vocabulary from `AGENTS.md` so future agent sessions know the canonical domain terms:
...
For Claude Code-oriented repositories, add this line to `CLAUDE.md`:

```markdown
@docs/ubiquitous-language.md
```

Keep the file under 200 lines when possible to reduce context pressure. If it grows beyond 200 lines, split into per-context files and reference each file from the repository's agent context document.
