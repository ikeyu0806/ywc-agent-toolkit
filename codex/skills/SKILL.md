...
description: "(ywc) Use when the user wants a UI/UX review combining static code analysis with live UI exploration covering information architecture, visual design, usability, and accessibility. Triggers: 'UI/UX review', 'UX 점검', 'UI 검토', 'usability audit', 'design review', 'accessibility review', '사용성 점검', '디자인 리뷰', 'UX 監査', 'UI レビュー'. Do not use for backend/API review (use ywc-impl-review), product/business strategy review (use ywc-product-review), or code-only review without a running UI."
---
...
Conduct a prioritized UI/UX review in Codex by combining static code analysis with live UI exploration. Focus areas: Information Architecture and Visual Design. Output: a four-tier (Critical / High / Medium / Low) Markdown report.
...
| "Reduced motion / dark mode are edge cases, drop them" | These are accessibility requirements, not edge cases. Test both when applicable. |
| "Severity feels Critical, mark it Critical regardless of impact" | Critical means blocks core flow or violates accessibility law. Reserve it. Inflated severity buries real critical findings. |
...
Use Codex file and shell tools to map the project. Prefer `rg`, `rg --files`, `find`, and targeted file reads:
...
Use the best available Codex browser capability for inspection-focused analysis. Prefer Playwright or a browser MCP when available; use Chrome DevTools MCP if the environment exposes it. If no browser tool is available, note the limitation and continue with code-only review.

Recommended browser evidence sequence:

1. Open the target URL in the browser tool.
2. Capture an accessibility tree or DOM snapshot when the tool supports it.
3. Capture screenshots per breakpoint.
4. Resize and repeat for 360 / 768 / 1280 / 1920.
5. Run automated accessibility or Lighthouse-style checks when available.
6. Inspect console messages for runtime errors.
7. Evaluate computed styles, contrast, and focus order when needed.
...
Generate the report using `assets/report-template.md` as the structural template. Fill all placeholders. Default output path: `codexdocs/ui-ux-review-{YYYY-MM-DD}.md` unless the user specifies otherwise.
