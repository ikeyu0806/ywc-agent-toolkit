...
"우리 Project의 UX, 어디부터 손봐야 하는가?"라는 질문에 evidence 기반으로 답하기 위한 Skill입니다. Code 정적 분석과 Codex에서 사용 가능한 browser automation 도구를 활용한 Live UI 탐색을 함께 수행하며, 모든 finding은 권위 있는 heuristic(Nielsen 10 / WCAG 2.2 AA / Material 3 / Apple HIG / Internal Design System)에 근거를 둡니다.
...
- **Evidence 효율**: 접근성 tree, screenshot, console, computed style 등 사용 가능한 browser evidence 우선 수집
...
| 2 | Code Reconnaissance | `rg`, `rg --files`, targeted file reads |
| 3 | Live UI Exploration | Playwright / browser MCP / Chrome DevTools MCP 등 사용 가능한 Codex 도구 |
...
기본 저장 경로는 `codexdocs/ui-ux-review-{YYYY-MM-DD}.md` 입니다.
...
본 Skill은 **Codex에서 사용 가능한 browser tooling**을 우선 사용합니다. Playwright, browser MCP, Chrome DevTools MCP 등 현재 환경에 노출된 도구를 사용하고, live browser 도구가 없으면 code-only review로 전환합니다.

| 작업 | 권장 Tool |
| --- | --- |
| Accessibility tree / DOM snapshot | Playwright / browser MCP / Chrome DevTools MCP |
| Screenshot 수집 | 사용 가능한 screenshot tool |
| Responsive 검증 | browser resize / viewport 설정 |
| 자동 accessibility scoring | Lighthouse 또는 접근성 audit tool |
| Computed style / contrast 계산 | browser evaluation tool |
| Console error 수집 | console inspection tool |

복잡한 multi-step interaction이 필요한 경우에는 Playwright 계열 도구를 우선 고려합니다.
...
- [ywc-task-generator](../ywc-task-generator/README.md) - 다른 Codex Skill 문서화 예시
