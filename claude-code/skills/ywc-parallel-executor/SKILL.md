...
| "Worktree cleanup is manual hassle, skip it" | Stale worktrees corrupt subsequent runs. Always cleanup after each task completes. |
...
| Category | Agent Type | Description |
|----------|-----------|-------------|
| `db`, `api`, `domain`, `lib`, `worker` | Backend Agent | Server-side code generation/modification |
| `ui` | Frontend Agent | UI component, page generation/modification |
| `test` | QA Agent | Test strategy + test code generation |
| `infra` | DevOps Agent | CI/CD, deployment configuration |
| `refactor` | Reviewer Agent | Code structure improvement |
...
```bash
bash tools/claude-code/skills/ywc-parallel-executor/scripts/cleanup-worktree.sh <task-name>
