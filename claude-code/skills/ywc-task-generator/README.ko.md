...
이 Claude Code Skill 은 Specification 을 dependency-safe 하고 reviewable 한 구현 Task 로 분해합니다.

일반적인 task decomposition 뿐 아니라, `git worktree` 와 분리된 Codex / Claude Code session 을 사용하는 병렬 실행까지 고려한 Task metadata 를 생성하도록 설계되어 있습니다.
...
> Primary Sources 에 외부 URL (Notion, Confluence, Figma 등) 을 넣는 경우 project-level policy 가 필요합니다. Default 는 project-relative paths only 이며, `sequential-executor` skill 이 정책을 `.claude/settings.local.json` 의 `taskExecutor.externalSpecUrls` 에 저장해 관리합니다.
