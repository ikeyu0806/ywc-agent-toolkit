...
静的 Code 分析と Chrome DevTools MCP を用いた Live UI 探索を組み合わせ、Project を UI/UX 観点で点検し、優先度別 Markdown report を生成する Claude Code Skill です。
...
本 Skill は inspection 中心の作業に Chrome DevTools MCP を優先使用します(accessibility tree snapshot、Lighthouse audit、computed style、screenshot 等)。Multi-step の interaction 自動化が必要な場合のみ Playwright MCP を補助的に使用します。
