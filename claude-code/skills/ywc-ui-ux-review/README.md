...
자신의 Project를 정적 Code 분석과 Live UI 탐색을 결합하여 UI/UX 관점에서 점검하고, 우선순위별 Markdown report를 생성하는 Claude Code Skill입니다.

## 개요

"우리 Project의 UX, 어디부터 손봐야 하는가?"라는 질문에 evidence 기반으로 답하기 위한 Skill입니다. Code 정적 분석과 Chrome DevTools MCP를 활용한 Live UI 탐색을 함께 수행하며, 모든 finding은 권위 있는 heuristic(Nielsen 10 / WCAG 2.2 AA / Material 3 / Apple HIG / Internal Design System)에 근거를 둡니다.
...
- **Token 효율**: Chrome DevTools MCP의 accessibility tree snapshot 우선 사용
...
본 Skill은 **Chrome DevTools MCP**를 우선 사용합니다.

| 작업 | 권장 Tool |
| --- | --- |
| Accessibility tree snapshot | `mcp__chrome-devtools__take_snapshot` |
| Screenshot 수집 | `mcp__chrome-devtools__take_screenshot` |
| Responsive 검증 | `mcp__chrome-devtools__resize_page` |
| 자동 accessibility scoring | `mcp__chrome-devtools__lighthouse_audit` |
| Computed style / contrast 계산 | `mcp__chrome-devtools__evaluate_script` |
| Console error 수집 | `mcp__chrome-devtools__list_console_messages` |
...
- [Claude Code Skills Guide](../../README.md)
